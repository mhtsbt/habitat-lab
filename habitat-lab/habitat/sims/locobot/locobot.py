...
class LocobotState:
    def __init__(self):
        self.position = np.zeros(shape=3, dtype=np.float)
        self.rotation = quaternion.from_rotation_vector(10)

@registry.register_sensor
class LocobotDepthSensor(DepthSensor, HabitatSimSensor):
    _get_default_spec = habitat_sim.CameraSensorSpec
    _config_ignore_keys = {
        "max_depth",
        "min_depth",
...
@registry.register_sensor
class LocobotEquirectangularDepthSensor(LocobotDepthSensor):
    _get_default_spec = habitat_sim.EquirectangularSensorSpec
...
@registry.register_sensor
class LocobotFisheyeDepthSensor(LocobotDepthSensor):
    _get_default_spec = habitat_sim.FisheyeSensorDoubleSphereSpec
    ...
    def __init__(self, config: DictConfig) -> None:
        ...
        for agent_config in self.habitat_config.agents.values():
            for sensor_cfg in agent_config.sim_sensors.values():
                sensor_type = registry.get_sensor(sensor_cfg.type.replace("HabitatSim", "Locobot"))
